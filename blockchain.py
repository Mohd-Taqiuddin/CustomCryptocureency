...
class Blockchain:
    ...
    def save_data(self):
        ...
        try:
            ...
            # svc = json.dumps(saveable_chain)
            # svc_dict ={}
            # print(f'***************{saveable_chain}*************')
            if len(saveable_chain) < 1:
                print('NO CHAIN')
            else:
                insert_block = saveable_chain[-1]
                print('***********insert_block**********')
                print(f'***************{insert_block}*************')
                db.blockchain.insert_one(insert_block)
            try:
                print('****************TRANS_SAVE***************')
                print(f'********{saveable_tx}**********')
                if self.__open_transactions != []:
                    db.transaction.insert_many(saveable_tx)
            except Exception as ex:
                print(f'**********{ex}************')
                print('SAVING TRANSACTIONS FAILED!!')
            # db.transaction.insert_many(saveable_tx)
            # try:
            #     print('****************NODES_SAVE***************')
            #     print(f'********{json.dumps(nodes)}**********')
            #     if nodes != []:
            #         db.transaction.insert_many(nodes)
            # except Exception as ex:
            #     print(f'**********{ex}************')
            #     print('SAVING NODES FAILED!!')
            # db.nodes.insert_one(json.dumps(nodes))
